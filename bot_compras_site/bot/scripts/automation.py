...
def abrir_site(): # abrindo um site no chromium
...
def barra_pesquisa(pesquisa): # usando a barra de pesquisa
...
def localizar_produto(): # localizando e escolhendo o produto
...
def localizar_sabor(sabor): # localizando e escolhendo o sabor do produto, caso necessário
...
def adicionar_carrinho(): # adicionando ao carrinho
...
def ir_carrinho(): # indo para o carrinho
...
def calcular_frete(cep): # calculando o frete
...
def preencher_cupom(cupom): # preenchendo o cupom de desconto, se houver
...
def ir_pagina_final(): # indo pra página de finalização de compra
...
def preencher_cpf(cpf): # preenchendo os dados pessoais
...
def inserir_senha(senha): # entrando na conta do usuário
...
def inserir_nome(nome): # criando uma conta para o usuário
...
def inserir_email(email): # insere email no campo de texto
...
def inserir_telefone(tel): # insere telefone no campo de texto
...
def inserir_sexo(sexo): # aperta o botão informando qual o sexo do usuário
...
def criar_senha(senha): # cria senha para a conta do usuário
...
def cadastrar_endereco(num_residencia): # preenchendo os dados de endereço, caso necessário
...
def ir_pagamento(): # indo para a aba de pagamento
...
def localizar_metodo_pag(metodo_pag): # preenchendo os dados de pagamento
...
def inserir_num_cartao(num_cartao): # insere o número do cartão de crédito
...
def inserir_nome_titular(nome_titular): # insere o nome do titular do cartão
...
def inserir_cpf_titular(cpf): # insere o CPF do titular
    campo_cpf_titular = driver.find_element(By.ID, 'docNumber') # localiza o campo de CPF do titular
    campo_cpf_titular.click()
    sleep(1)
    campo_cpf_titular.send_keys(cpf)
    sleep(1)

def inserir_mes_validade(mes): # seleciona o mês de validade do cartão
...
def inserir_ano_validade(ano): # seleciona o ano de validade do cartão
    from selenium.webdriver.support.ui import Select
...
def inserir_cod_seg(cod_seg): # localiza o campo de código de segurança
...
def inserir_campo_parcelas(parcelas): # seleciona o número de parcelas  
    from selenium.webdriver.support.ui import Select
...
def finalizar_compra(): # finaliza compra
